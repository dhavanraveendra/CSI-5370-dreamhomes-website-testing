...
if (isset($_POST['update'])) {
    ...
    $t_amount = mysqli_real_escape_string($conn, $_POST['t_amount']);

    // Update the transaction information in the database
    ...
    if (!$result) {
        ...
        header('location: admin_transactions.php'); // Redirect to the transactions list page
    }
}

// Fetch the transaction information based on the T_ID parameter
if (isset($_GET['T_ID'])) {
    $T_ID = $_GET['T_ID'];
    $sql = "SELECT * FROM Transaction WHERE T_ID = $T_ID";
    $result = $conn->query($sql);

    if ($result->num_rows == 1) {
        $row = $result->fetch_assoc();
        // Display the edit form with pre-filled data
        include('admin_edit_transaction_form.php');
    } else {
